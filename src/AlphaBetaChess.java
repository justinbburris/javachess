...
public class AlphaBetaChess {
  ...
  public static void makeMove(String move) {
    if(move.charAt(4) != 'P') {
      ...
      if("K".equals(chessBoard[Character.getNumericValue(move.charAt(2))][Character.getNumericValue(move.charAt(3))])) {
        kingPositionW = 8 + Character.getNumericValue(move.charAt(2)) + Character.getNumericValue(move.charAt(3));
      }
    } else { 
      // if pawn promotion
  ...
  public static void undoMove(String move) {
    if(move.charAt(4) != 'P') {
      ...
      if("K".equals(chessBoard[Character.getNumericValue(move.charAt(0))][Character.getNumericValue(move.charAt(1))])) {
        kingPositionW = 8 + Character.getNumericValue(move.charAt(0)) + Character.getNumericValue(move.charAt(1));
      }
    } else { 
      // if pawn promotion
      //column1, column2, captured-piece, new-piece, P
  ...
  public static String possibleQ(int pos) {
    String list = "", oldPiece;
    ...
    for(int i = -1; i <= 1; i++) {
      for(int j = -1; j <= 1; j++) {
        try {
          int newRank = rank + temp * i, 
                  newFile = file + temp * j;
          oldPiece = chessBoard[newRank][newFile];
          
          while((" ").equals(oldPiece)) {
            chessBoard[rank][file] = " ";
            chessBoard[newRank][newFile] = "Q";
            
            if(kingSafe()) {
              list = list + rank + file + (newRank) + (newFile) + oldPiece;
            }
            
            // reset
            chessBoard[rank][file] = "Q";
            chessBoard[newRank][newFile] = oldPiece;
            temp++;
            newRank = rank + temp * i;
            newFile = file + temp * j;
            oldPiece = chessBoard[newRank][newFile];
          ...
          oldPiece = chessBoard[newRank][newFile];
          // Duplicate code :(
          if(Character.isLowerCase(oldPiece.charAt(0))) {
            chessBoard[rank][file] = " ";
            chessBoard[newRank][newFile] = "Q";
            if(kingSafe()) {
              list = list + rank + file + (newRank) + (newFile) + oldPiece;
  ...
  public static boolean kingSafe() {
    ...
    for (int i=-1; i<=1; i+=2) {
      for (int j=-1; j<=1; j+=2) {
        try {
          while(" ".equals(chessBoard[kingPositionW / 8 +  temp * i][kingPositionW%8+temp*j])) {temp++;}
    ...
    for (int i=-1; i<=1; i+=2) {
      try {
        while(" ".equals(chessBoard[kingPositionW / 8][kingPositionW%8+temp*i])) {temp++;}
      ...
      temp = 1;
      try {
        while(" ".equals(chessBoard[kingPositionW / 8+temp*i][kingPositionW%8])) {temp++;}
    ...
    for (int i=-1; i<=1; i+=2) {
      for (int j=-1; j<=1; j+=2) {
        try {
          if("n".equals(chessBoard[kingPositionW / 8+i][kingPositionW%8+j*2])) {
            return false;
          }
        } catch(Exception e) {}
        try {
          if("n".equals(chessBoard[kingPositionW / 8+i*2][kingPositionW%8+j])) {
            return false;
          }
        } catch(Exception e) {}
      }
    ...
    if(kingPositionW >= 16) {
      try {
        if("p".equals(chessBoard[kingPositionW / 8-1][kingPositionW%8-1])) {
          return false;
        }
      } catch(Exception e) {}
      try {
    ...
    for (int i=-1; i<=1; i++) {
      for (int j=-1; j<=1; j++) {
        if(i != 0 || j != 0) {
          try {
            if("k".equals(chessBoard[kingPositionW / 8+i][kingPositionW%8+j])) {
              return false;
            }
          } catch(Exception e) {}
        }
